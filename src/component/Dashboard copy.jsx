...
const Dashboard = () => {
  ...
  const [roomCounters, setRoomCounters] = useState(() => {
    return getLocalStorageItem("room_counters", {});
  });
  ...
  useEffect(() => {
    ...
    const handleSettings = (data) => {
      ...
      // 🟢 ONLINE ROOM EVENT
      if (data.en === "ONLINE_ROOM_COUNTER") {
        const rooms = data?.data?.OnlineCounterInfo || [];

        const formatted = {};
        rooms.forEach((room) => {
          formatted[room.roomId] = {
        ...
        setRoomCounters(formatted);
        localStorage.setItem("room_counters", JSON.stringify(formatted));
      }
      ...
      <section className="w-full min-w-full sm:max-w-md p-4 rounded-lg shadow-lg text-center relative">
        ...
        <div className="grid grid-cols-4 gap-4 mt-4 mb-6">
          ...
          <Link to="/live/table1" className="flex flex-col items-center group">
            ...
            <p className="text-xs sm:text-sm">
              Opening Time: {roomCounters?.table1?.start_time ?? "--"}
            </p>
            <p className="text-xs sm:text-sm">
              Closing Time: {roomCounters?.table1?.close_time ?? "--"}
            </p>
          </Link>
          ...
          <Link to="/live/table2" className="flex flex-col items-center group">
            ...
            <p className="text-xs sm:text-sm">
              Opening Time: {roomCounters?.table2?.start_time ?? "--"}
            </p>
            <p className="text-xs sm:text-sm">
              Closing Time: {roomCounters?.table2?.close_time ?? "--"}
            </p>
          </Link>
